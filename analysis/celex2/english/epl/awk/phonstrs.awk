...
# with stress markers, in any of the formats (SAM-PA, CELEX, CPA) 
...
BEGIN {
        ...
        if (ARGC <= 2) {
                ...
                printf "USAGE !!\n awk -f phonstrs.awk file LexField_file1 (SP|CX|CP)\n"
                exit(-1)
	      }
	  ...
	  LexInfo_1 = $ARGV[2];
          ...
          if (ARGV[3] == "CP") {
              Charset = ARGV[3];
              LexInfo_1 = ChangeRepresentation(SyllableMarkersToFullStops(LexInfo_1));
	    }
          else if (ARGV[3] == "SP" || ARGV[3] == "CX") {
              Charset = ARGV[3];
              LexInfo_1 = ChangeRepresentation(LexInfo_1);
	    }
	  printf("%s\n",LexInfo_1);
        }
}
