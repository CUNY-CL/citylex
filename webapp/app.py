...
def _neg_logprob(raw_freq: float, total_words: float) -> float:
    if raw_freq > 0:
...
def _flatten(maybe_list: Any) -> Any:
...
def _data_to_tsv(
    ...
    """Fetches and writes data from the specified SQL table for given columns.
...
def _wikipron_data_to_tsv(
    ...
    # Ensures uk_or_us is capitalized as "UK" or "US".
    cursor.execute(
    ...
    try:
        ...
        if output_format == "long":
            ...
            if any(
                ...
                celex_wordforms_data: Dict[str, Any] = {}
                ...
                # Writes consolidated CELEX data to TSV.
                for wordform, data in celex_wordforms_data.items():
                    ...
                    if (
                        "celex_DISC" in selected_fields
                        and "pronunciation" in data
                    ):
                        row["DISC_pronunciation"] = data["pronunciation"]
                    # Skips rows with only wordform and source.
                    if len(row) < 3:
                        continue
                    writer.writerow(row)
            # Fetches and writes SUBTLEX-UK data.
            if "subtlexUK" in selected_sources:
                _subtlex_data_to_tsv(cursor, writer, selected_fields, "UK")
            # Fetches and writes SUBTLEX-US data.
            if "subtlexUS" in selected_sources:
                _subtlex_data_to_tsv(cursor, writer, selected_fields, "US")
            ...
            if "UDLexicons" in selected_sources:
                ...
                for wordform, source, ud_tags in cursor:
                    ...
                    # If a requested mapping is missing, skips the row.
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f"citylex-{isodate}.tsv",
            )
        # JSON option.
        elif output_format == "wide":
            aggregated_data: Dict[str, Dict[str, Set[Any]]] = {}

            def add_to_aggregated_data(
                wordform: str, key: str, value: Any
            ) -> None:
                try:
            ...
            for source, source_fieldname in [
                ...
                if source_fieldname in selected_sources:
                    ...
                    for wordform, raw_frequency, freq_per_million in cursor:
                        if (
                            f"{source_fieldname}_raw_frequency"
                            in selected_fields
                        ):
                        ...
                        if f"{source_fieldname}_zipf" in selected_fields:
                            zipf_val = zipf.zipf_scale(
                                raw_frequency, total_words
                            )
                            add_to_aggregated_data(
                                wordform,
                                f"{source} (Zipf scale)",
            ...
            if "UniMorph" in selected_sources:
                ...
                for wordform, um_tags in cursor:
                    ...
                    if "um_UMtags" in selected_fields:
                        add_to_aggregated_data(
                            wordform, "UniMorph features", um_tags
                        )
            ...
            if "celexfeat" in selected_sources:
                ...
                for wordform, celex_tags in cursor:
                    ...
                    if "celex_CELEXtags" in selected_fields:
                        add_to_aggregated_data(
                            wordform, "CELEX features", celex_tags
                        )
            ...
            for source_key, field_prefix, display_name in [
                ...
                if source_key in selected_sources:
                    ...
                    for wordform, pronunciation in cursor:
                        # Checks if this pronunciation field was selected.
                        if (
                            (
            ...
            return send_file(
                ...
                as_attachment=True,
                download_name=f"citylex-{isodate}.json",
            )
    finally:
        conn.close()
