...
# This script converts a word into lowercase after stripping off all
# (already split off) diacritics and non-alphanumeric characters.

BEGIN{
  if(ARGC != 3){
    printf "insufficient number of arguments! (%d)\n", ARGC-1
      printf "USAGE !!\n awk -f tolower.awk file1 LexField_file1\n"
	exit(-1)
  }
...
# InitializeToLower(Map);
  ...
  while(getline < ARGV[1]){
    LexInfo_1 = $ARGV[2];
    LexInfo_1 = ToLower(StripNonAlphabetics(StripDiacritics(LexInfo_1)));
    printf("%s\n",LexInfo_1);
  }
}
