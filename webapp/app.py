...
def _csv_row(
    ...
    """Writes one DictWriter row and returns the buffered string, then clears."""
    writer.writerow(row)
    value = buf.getvalue()
    buf.seek(0)
    buf.truncate(0)
...
def post() -> Response | tuple[str, int]:
    ...
    if output_format == "long":
        ...
        return _stream_tsv_response(
            selected_sources, selected_fields, columns, today
        )
    elif output_format == "wide":
        return _stream_json_response(selected_sources, selected_fields, today)
    return render_template("400.html"), 400
